...
Lite 版与完整 Tuner 共用编辑器和攻击拖尾能力，但使用独立服务、独立项目列表和独立数据目录，不会修改 Godot 项目，也不会改变完整 Tuner 当前选中的项目。角色/动画/单帧变换、真实帧时长、disabled frame、碰撞框元数据、图片图层与攻击拖尾都可照常编辑保存；只有音效、Godot runtime 与游戏绑定明确关闭。
...
导入阶段不要求用户预先决定统一画布。先在页面里校准当前角色的所有动作、图层和拖尾；“透明序列导出”会扫描该角色全部主动作组的实际可见像素范围，加入可调透明边距，再自动得到一个不会裁切且尽量紧凑的全角色统一画布。所有动作共享这个尺寸和同一个角色原点，切换动作时不会跳位。点击任一导出按钮都会先打开系统文件夹选择器，不会写入固定的 Lite 内部路径。“导出 PNG 序列”为当前角色的每个主动作分别写逐帧透明 PNG；“导出 Sheet + JSON”为每个主动作分别写 `spritesheet.png` 和可被 Lite 重新导入的 `spritesheet.json`。附属图层会合成进所属主动作，不会被重复导出成另一组。
